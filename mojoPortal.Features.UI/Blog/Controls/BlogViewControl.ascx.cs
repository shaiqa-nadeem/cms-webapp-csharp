...
public partial class BlogViewControl : UserControl, IRefreshAfterPostback, IUpdateCommentStats
{
	...
	protected virtual void PopulateControls()
	{
		...
		var attachments = FileAttachment.GetListByItem(blog.BlogGuid);
	...
	private void btnPostComment_Click(object sender, EventArgs e)
	{
		...
		{
			//having this information helps with banning IP Addresses of spammers
			string message = @$"
{basePage.SiteRoot}{blog.ItemUrl.Replace("~", string.Empty)}
HTTP_USER_AGENT: {Page.Request.ServerVariables["HTTP_USER_AGENT"]}
HTTP_HOST: {Page.Request.ServerVariables["HTTP_HOST"]}
REMOTE_HOST: {Page.Request.ServerVariables["REMOTE_HOST"]}
REMOTE_ADDR: {SiteUtils.GetIP4Address()}
LOCAL_ADDR: {Page.Request.ServerVariables["LOCAL_ADDR"]}
HTTP_REFERER: {Page.Request.ServerVariables["HTTP_REFERER"]}
";
	...
	private void LoadSettings()
	{
		...
		searchBoxTop.Visible = config.ShowBlogSearchBox && !displaySettings.HideSearchBoxInPostDetail && !displaySettings.ShowSearchInNav;
		...
		authorAvatar.Email = blog.UserEmail;
		authorAvatar.UserName = blog.UserName;
		authorAvatar.UserId = blog.UserId;
		authorAvatar.AvatarFile = blog.UserAvatar;
		authorAvatar.MaxAllowedRating = MaxAllowedGravatarRating;
		authorAvatar.Disable = disableAvatars;
		authorAvatar.UseGravatar = allowGravatars;
		authorAvatar.SiteId = basePage.SiteInfo.SiteId;
		authorAvatar.UserNameTooltipFormat = displaySettings.AvatarUserNameTooltipFormat;
		authorAvatar.UseLink = UseProfileLinkForAvatar();
		authorAvatar.SiteRoot = SiteRoot;
		...
		bool showNav = (config.ShowArchives
	...
	private void SetupInternalCommentSystem()
	{
		...
		comments.EditBaseUrl = $"{SiteRoot}/Blog/CommentDialog.aspx?pageid={PageId.ToInvariantString()}&mid={ModuleId.ToInvariantString()}&ItemID={ItemId.ToInvariantString()}";
		...
		if (module != null)
		{
			if (Page.Master != null)
			{
				...
				if (head != null)
				{
					...
					var rssLink = new Literal
					{
						Text = $"<link rel=\"alternate\" type=\"application/rss+xml\" title=\"{module.ModuleTitle}\" href=\"{GetRssUrl()}\" />"
					};
	...
	private string GetRssUrl()
	{
		...
		{
			return $"{SiteRoot}/Blog/RSS.aspx?p={PageId.ToInvariantString()}~{ModuleId.ToInvariantString()}~-1&amp;r={Global.FeedRedirectBypassToken.ToString()}";
		}

		return $"{SiteRoot}/Blog/RSS.aspx?p={PageId.ToInvariantString()}~{ModuleId.ToInvariantString()}~-1";
	}
